...
講義設計AIは、真の生徒内部状態を直接見ない。伝達AIがまとめた観察可能情報と、それに基づいて更新された `teacher_beliefs` だけを使い、クラス全体に対する講義目標、時間配分、全体方略、個別支援方針を提案する。
...
問題難易度別に正答率と平均正答確率を見る。

論文では、同じ理解度設定でも、難易度1、2、3で正答確率が変化することを示す表として使える。これにより、単に理解度だけで正答率を決めているのではなく、問題側の条件も認知モデルに反映されていることを説明できる。

### Parameter Guide

各パラメータの意味と観測される効果を見る。

論文説明や発表でパラメータについて深掘りされた場合は、数式の詳細だけでなく、`Parameter Guide` と各評価表を対応させて説明する。
